...
<div class="container mt-4" style="max-width:600px">
    ...
    <form action="{{ route('materias.update', $materia) }}" method="POST" class="card p-4 bg-white">
        @csrf
        @method('PUT')
        <div class="mb-3">
            <label class="form-label">Sigla</label>
            <input type="text" name="sigla" class="form-control text-uppercase"
                   value="{{ $materia->sigla }}" required maxlength="20">
        </div>
        <div class="mb-3">
            <label class="form-label">Nombre</label>
            <input type="text" name="nombre" class="form-control"
                   value="{{ $materia->nombre }}" required maxlength="100">
        </div>
        <div class="mb-3">
        ...
        <div class="row">
            <div class="col-md-4 mb-3">
                ...
                <input type="number" name="peso_examen1" class="form-control"
                       value="{{ $materia->peso_examen1 }}" required min="1" max="98" step="0.01"
                       oninput="calcularSuma()">
            </div>
            <div class="col-md-4 mb-3">
                <label class="form-label">Examen 2 (%)</label>
                <input type="number" name="peso_examen2" class="form-control"
                       value="{{ $materia->peso_examen2 }}" required min="1" max="98" step="0.01"
                       oninput="calcularSuma()">
            </div>
            <div class="col-md-4 mb-3">
                <label class="form-label">Examen 3 (%)</label>
                <input type="number" name="peso_examen3" class="form-control"
                       value="{{ $materia->peso_examen3 }}" required min="1" max="98" step="0.01"
                       oninput="calcularSuma()">
            </div>
        </div>
        <div id="suma-pesos" class="alert alert-success py-2">
            Total: <strong id="total-pesos">100</strong>%
            <span id="suma-ok">✅ Correcto</span>
        </div>
        <button type="submit" class="btn btn-success w-100">Actualizar Materia</button>
    </form>
</div>
<script>
